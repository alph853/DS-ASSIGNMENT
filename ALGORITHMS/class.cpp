...
std::pair<double, double> haversineDistanceAndBearing(const node &A, const node &B)
{
   ...
   double c = 2 * std::asin(std::sqrt(std::pow(std::sin(dlat / 2), 2) + std::cos(lat1) * std::cos(lat2) * std::pow(std::sin(dlon / 2), 2)));
...
void operate::loadRoad(const std::string &roadFile) const
{
   m_nodeManager->loadRoad(roadFile);
}

void operate::loadNode(const std::string &nodeFile) const
{
...
// nodeManager
void nodeManager::loadNode(const std::string &nodeFile)
{
...
void nodeManager::loadRoad(const std::string &roadFile)
{
...
std::vector<std::string> nodeManager::printPath(const matrix<int> &paths) const
{
   ...
   for (const auto &p : paths)
   {
      ...
      result.push_back("Path: " + str + std::string(50, '-') + '\n' + "Total Distance: " + stream.str() + "km\n" + std::string(50, '-') + '\n' + outstr + '\n');
   }
   return result;
}
